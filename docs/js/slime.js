...
export default class Slime{
	...
	handleCol(colorData){
		for(let i = 3; i < colorData.length; i+=4){
			if(colorData[i] != 0){
				if(colorData[i-3] == "255" && colorData[i-2] == "0" && colorData[i-3] == "255"){this.kill(); break};
			}
		}
	}	
	...
	moveLeft(){
		if(this.x>0){this.x-=3}else{this.dir = 'right'};
	}
	moveRight(){
		if(this.x<1900){this.x+=3}else{this.dir = 'left'};
	}
